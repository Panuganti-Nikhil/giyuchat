  ...
  return (
    <div className={`min-h-screen flex items-center justify-center relative overflow-hidden ${isDark ? 'bg-app-dark' : 'bg-app-light'}`}>
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute w-[500px] h-[500px]  bg-violet-600/10 blur-[120px] -top-60 -left-60 animate-glow" />
        <div className="absolute w-[400px] h-[400px]  bg-blue-500/10 blur-[100px] -bottom-40 -right-40 animate-glow-delayed" />
      </div>

      <div className="relative z-10 w-full max-w-md mx-4 animate-slide-up">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16  premium-btn mb-4  -violet-500/20">
        ...
        <div className={` p-6 sm:p-8  ${isDark ? 'glass' : 'glass-light'}`}>
          {!isAuthenticated ? (
            ...
            <div className="space-y-4">
              <div className={`text-center py-2.5  ${isDark ? 'bg-white/5' : 'bg-violet-50'}`}>
              ...
              {!pinRoom && (
                <>
                  {/* Tabs */}
                  <div className={`flex  p-1 ${isDark ? 'bg-white/5' : 'bg-black/5'}`}>
                    <button onClick={() => { setTab('create'); setJoinMode(null); setError(''); }}
                      className={`flex-1 py-2.5  text-sm font-medium transition-all flex items-center justify-center gap-2
                        ${tab === 'create' ? 'premium-btn text-white ' : isDark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-700'}`}>
                      <Plus className="w-4 h-4" /> Create
                    </button>
                    <button onClick={() => { setTab('join'); setError(''); }}
                      className={`flex-1 py-2.5  text-sm font-medium transition-all flex items-center justify-center gap-2
                        ${tab === 'join' ? 'premium-btn text-white ' : isDark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-700'}`}>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className={`flex  p-1 ${isDark ? 'bg-white/5' : 'bg-black/5'}`}>
                        <button onClick={() => { setIsPublic(false); setPinEnabled(false); }}
                          className={`flex-1 py-2.5  text-sm font-medium transition-all flex items-center justify-center gap-2
                            ${!isPublic ? 'bg-violet-600 text-white ' : isDark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-700'}`}>
                          <Key className="w-3.5 h-3.5" /> Private
                        </button>
                        <button onClick={() => setIsPublic(true)}
                          className={`flex-1 py-2.5  text-sm font-medium transition-all flex items-center justify-center gap-2
                            ${isPublic ? 'bg-emerald-600 text-white ' : isDark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-700'}`}>
                      ...
                      {isPublic && (
                        <>
                          ...
                          <div className={`flex items-center justify-between px-3 py-2  ${isDark ? 'bg-white/5' : 'bg-black/5'}`}>
                            <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => setPinEnabled(!pinEnabled)}
                              className={`w-10 h-5  flex items-center transition-all ${pinEnabled ? 'bg-amber-500 justify-end' : isDark ? 'bg-white/10 justify-start' : 'bg-black/10 justify-start'}`}>
                          ...
                          {pinEnabled && (
                            <input type="text" value={pin} onChange={e => setPin(e.target.value.replace(/\D/g, '').slice(0, 4))}
                              ...
                              className={`w-full px-4 py-2.5  outline-none transition-all tracking-[0.3em] text-center font-mono
                      ...
                      <div className={`flex items-center justify-between px-3 py-2  ${announcementOnly ? 'bg-blue-500/10 border border-blue-500/20' : isDark ? 'bg-white/5' : 'bg-black/5'}`}>
                        <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => setAnnouncementOnly(!announcementOnly)}
                          className={`w-10 h-5  flex items-center transition-all ${announcementOnly ? 'bg-blue-500 justify-end' : isDark ? 'bg-white/10 justify-start' : 'bg-black/10 justify-start'}`}>
                          <div className="w-4 h-4 bg-white  mx-0.5 " />
                        </button>
                      </div>
                      ...
                      {joinMode === null ? (
                        <div className="space-y-3">
                          <button onClick={() => setJoinMode('global')}
                            className={`w-full py-4  border transition-all flex items-center gap-4 px-4
                          ...
                          <button onClick={() => setJoinMode('code')}
                            ...
                            <div className="w-10 h-10  bg-violet-500/20 flex items-center justify-center flex-shrink-0">
                        ...
                        <div className="space-y-3">
                          ...
                          <input id="room-code-input" type="text" value={roomCode}
                            ...
                            className={`w-full px-4 py-3  outline-none transition-all tracking-[0.3em] text-center font-mono text-lg
                              ${isDark ? 'bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:border-violet-500/50'
                                : 'bg-black/5 border border-black/10 text-slate-900 placeholder-slate-400 focus:border-violet-500/50'}`} />
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={fetchPublicRooms} disabled={loadingRooms}
                              className={`p-1.5  transition-colors ${isDark ? 'text-slate-400 hover:text-white hover:bg-white/10' : 'text-slate-500 hover:text-slate-800 hover:bg-black/10'} ${loadingRooms ? 'animate-spin' : ''}`}>
                              <RefreshCw className="w-4 h-4" />
                            </button>
                          </div>

                          <div className={`flex items-center gap-2 px-3 py-2  border ${isDark ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/10'}`}>
                              ...
                              filteredRooms.map(room => (
                                <button key={room.code} onClick={() => handleJoinPublic(room)}
                                  ...
                                  <div className="flex items-center justify-between mb-1">
                                    ...
                                    <span className="text-xs font-mono px-1.5 py-0.5  bg-violet-500/20 text-violet-400 flex-shrink-0 ml-2">
                                      JOIN
                                    </span>
                                  </div>
                                  {(room.tags || []).length > 0 && (
                                    <div className="flex gap-1 mb-1">
                                      {room.tags.map(t => (
                                        <span key={t} className="text-[10px] px-1.5 py-0.5  bg-violet-500/10 text-violet-400">{t}</span>
                                      ))}
                                    </div>
                                  )}
