  ...
  return (
    <>
      ...
      `}>
        ...
        <button onClick={onClose} className={`p-1.5  ${isDark ? 'hover:bg-white/10 text-slate-400' : 'hover:bg-black/10 text-slate-600'}`}>
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {mode === 'members' && (
          <div className="space-y-2">
            {members.map(m => {
              ...
              return (
                <div key={m.username} className={`p-3  border flex flex-col gap-2 
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <div className="relative">
                      ...
                      <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5  border-2
                  ...
                  {canManage && (
                    <div className="flex flex-wrap gap-1.5 mt-1 border-t border-white/5 pt-2">
                      {isOwner && m.role !== 'admin' && m.role !== 'owner' && (
                         <button onClick={() => onPromote(m.username)} className="text-[10px] px-2 py-1  bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/30">Promote to Admin</button>
                      )}
                      {(isOwner || (userRole === 'admin' && m.role !== 'admin' && m.role !== 'owner')) && (
                         <button onClick={() => onKick(m.username)} className="text-[10px] px-2 py-1  bg-red-500/20 text-red-400 hover:bg-red-500/30">Kick User</button>
                      )}
                      {isOwner && m.role === 'admin' && (
                         <button onClick={() => onDemote(m.username)} className="text-[10px] px-2 py-1  bg-orange-500/20 text-orange-400 hover:bg-orange-500/30">Demote to Member</button>
                      )}
                      {isOwner && (
                         <button onClick={() => setTransferTarget(m.username)} className="text-[10px] px-2 py-1  bg-amber-500/20 text-amber-400 hover:bg-amber-500/30">Transfer Ownership</button>
                      )}
                    </div>
                  )}
                </div>
        ...
        {mode === 'settings' && (
          <div className="space-y-5">
            {/* Dark Mode */}
            <div className={`p-3  flex items-center justify-between ${isDark ? 'bg-white/5' : 'bg-black/5'}`}>
              ...
              <button onClick={toggle} className="p-2  premium-btn text-white">
            ...
            <div className={`p-3  flex items-center justify-between ${isDark ? 'bg-white/5' : 'bg-black/5'}`}>
              ...
              <button onClick={onToggleSound} className={`p-2  text-white ${soundEnabled ? 'premium-btn' : 'bg-slate-500'}`}>
            ...
            <button 
              ...
              className="w-full p-3  flex items-center justify-between text-left transition-colors bg-blue-500/10 hover:bg-blue-500/20 text-blue-500 border border-blue-500/20"
            >
              <div className="flex flex-col">
            ...
            {/* Change Username */}
            <div className={`p-3  space-y-2 ${isDark ? 'bg-white/5' : 'bg-black/5'}`}>
              <span className={`text-sm font-medium ${isDark ? 'text-white' : 'text-slate-800'}`}>Change Username</span>
              <div className="flex gap-2">
                <input 
                  value={nameInput} 
                  onChange={(e) => setNameInput(e.target.value)}
                  className={`flex-1 px-3 py-1.5 text-sm  outline-none ${isDark ? 'bg-black/20 text-white' : 'bg-white/50 text-black'}`}
                  placeholder="New username"
                />
                <button onClick={handleUpdateName} className="p-1.5  premium-btn text-white"><Save className="w-4 h-4" /></button>
              </div>
            </div>

            {/* Room Background (Admin Only) */}
            {(userRole === 'owner' || userRole === 'admin') && (
              <div className={`p-3  space-y-2 ${isDark ? 'bg-white/5 border border-violet-500/20' : 'bg-violet-50 border border-violet-500/20'}`}>
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    className={`flex-1 min-w-0 px-3 py-2 text-xs  outline-none ${isDark ? 'bg-black/20 text-white' : 'bg-white/50 text-black'}`}
                  />
                  <button onClick={handleApplyBg} className="px-4 py-2 shrink-0  premium-btn text-white text-xs font-semibold">Apply</button>
                </div>
              </div>
            )}
            ...
            {(userRole === 'owner' || userRole === 'admin') && (
              <div className={`p-3  flex items-center justify-between ${room?.burnerMode ? 'bg-orange-500/10 border border-orange-500/20' : isDark ? 'bg-white/5' : 'bg-black/5'}`}>
                <div className="flex flex-col">
                ...
                <button onClick={onToggleBurner}
                  className={`w-10 h-5  flex items-center transition-all ${room?.burnerMode ? 'bg-orange-500 justify-end' : isDark ? 'bg-white/10 justify-start' : 'bg-black/10 justify-start'}`}>
            ...
            <button
              onClick={onSignOut}
              className="w-full p-3  flex items-center justify-center gap-2 text-red-500 bg-red-500/10 hover:bg-red-500/20 transition-colors text-sm font-medium"
            >
      ...
      {transferTarget && (
        <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className={`p-4   w-full max-w-sm ${isDark ? 'glass bg-black/50' : 'glass-light bg-white/50'}`}>
             ...
             <div className="flex gap-2">
               <button onClick={() => setTransferTarget(null)} className={`flex-1 py-2 text-sm  ${isDark ? 'bg-white/10 text-white' : 'bg-black/10 text-black'}`}>Cancel</button>
               <button onClick={() => { onTransferOwnership(transferTarget); setTransferTarget(null); }} className="flex-1 py-2 text-sm  bg-red-500 text-white">Transfer</button>
             </div>
          </div>
        </div>
      )}
